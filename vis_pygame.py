...
class visualiser:
    ...
    def show(self, vis_data, turn=0, game=None, titleStr=None):
        ...
        if self.playerStrings is not None:
            ...
            label = self.font.render("cleaned: %d" %
                                     stats['cleaned'][0], 1, self.BLUE)
            self.screen.blit(label, (self.marginX - 100, self.marginY + 30))

            if len(self.playerStrings) > 1:
                label = self.font.render(
                    self.playerStrings[1], 1, self.MAGENTA)
                self.screen.blit(
                    label, (self.marginX + (self.X * self.unit) + 10, self.marginY + 10))
                label = self.font.render("cleaned: %d" %
                                         stats['cleaned'][1], 1, self.MAGENTA)
                self.screen.blit(
                    label, (self.marginX + (self.X * self.unit) + 15, self.marginY + 30))
        ...
        for y, x, r, e, b, p in cleaners:
            ...
            pygame.draw.circle(self.screen, c, (self.marginX + x * self.unit + centre_offset,
                               self.marginY + y * self.unit + centre_offset), self.cleaner_size)
            ...
            pygame.draw.circle(self.screen, c, (self.marginX + x * self.unit + centre_offset +
                               xo*offset, self.marginY + y * self.unit + centre_offset + yo * offset), radius)
